...
generatePattern
  :: TypeEnv -> Type -> Pattern -> GenerateM Error (Type, Constraints, TypeEnv)
...
generatePattern env st (BoolPat _) = do
  let c = st :~: TBool
  pure (TBool, [c], env)
generatePattern env st UnitPat = do
  let c = st :~: TUnit
  pure (TUnit, [c], env)
...
generatePattern env st (TuplePat pats) = do
  ...
  beta <- TVar <$> fresh
  let betaConstraints = [beta :~: st, beta :~: mkTupleType patTypes]
...
generatePattern env st (ListPat []) = do
  ...
  beta     <- TVar <$> fresh
  let betaConstraints = [beta :~: st, beta :~: list elemType]
...
generatePattern env st (ListPat pats) = do
  ...
  -- generate a fresh variable for the type of the whole list pattern
  beta <- TVar <$> fresh
  let betaConstraints = [beta :~: st, beta :~: list (head patTypes)]
  pure
    (beta, betaConstraints <> listConstraints <> patConstraints, env <> patEnvs)
  ...
  Just (Forall as kt) -> do
    ...
    case result of
      (resultTy, tyconargs) -> do
        let scrutConstraint = resultTy :~: st
        ...
        -- constructor
        let patEqualityConstraints = zipWith (:~:) patTypes tyconargs
        -- generate a fresh variable for the type of the whole pattern
        beta <- TVar <$> fresh
        let betaConstraint = [beta :~: st, beta :~: resultTy]
        pure
          ( beta
          , scrutConstraint
          :  betaConstraint
...
generateSubpatterns
  :: TypeEnv -> [Pattern] -> GenerateM Error ([Type], Constraints, TypeEnv)
