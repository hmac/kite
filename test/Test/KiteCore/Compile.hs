...
import AST (ExprT (..), Pat (..))
import Control.Monad.Identity (Identity)
import Control.Monad.State.Strict (evalState, get, put)
import Data.Name (Name)
import qualified Data.Set as Set
import Data.String (fromString)
import KiteCore.Compile (liftAllLambdas, liftLambda, liftLet)
import Test.Diff (shouldBe)
import Test.Hspec hiding (shouldBe)
import Type.Type (Type (TOther), Type' (Fn, IFn, UType), U (..))
...
test_liftLambda = describe "liftLambda" $ do
  it "lifts a simple lambda" $ do
    ...
    liftLambda "f" (TOther (Fn a a)) [VarPat a "x"] (VarT a "x") `shouldBe` ([VarPat a "x"], VarT (TOther (Fn a a)) "f")
  it "lifts a lambda with a different return type" $ do
    -- (x y -> x) : a -> b -> a
    let a = TOther $ UType (U 1 "a")
        b = TOther $ UType (U 2 "b")
    liftLambda "f" (TOther (Fn a (TOther (Fn b a)))) [VarPat a "x", VarPat b "y"] (VarT a "x")
      `shouldBe` ([VarPat a "x", VarPat b "y"], VarT (TOther (Fn a (TOther (Fn b a)))) "f")
  it "lifts a lambda with a free variable" $ do
    let a = TOther $ UType (U 1 "a")
        b = TOther $ UType (U 2 "b")
    liftLambda "f" (TOther (Fn b a)) [VarPat b "y"] (VarT a "x")
      `shouldBe` ([VarPat a "x", VarPat b "y"], AppT (TOther (Fn b a)) (VarT (TOther (Fn a (TOther (Fn b a)))) "f") (VarT a "x"))
  it "lifts a lambda with several bound and free variables" $ do
    let a = TOther $ UType (U 1 "a")
        ...
        -- e = f y (g x) : c
        -- result = func g f
        e = AppT c (AppT (TOther (Fn d c)) (VarT fType "f") (VarT b "y")) (AppT d (VarT (TOther (Fn a d)) "g") (VarT a "x"))
    let (bindings, application) = liftLambda "h" lamType [VarPat a "x", VarPat b "y"] e

    bindings `shouldBe` [VarPat fType "f", VarPat gType "g", VarPat a "x", VarPat b "y"]
    application `shouldBe` AppT lamType (AppT (TOther (Fn gType lamType)) (VarT (TOther (Fn fType (TOther (Fn gType lamType)))) "h") (VarT fType "f")) (VarT gType "g")
...
test_liftAllLambdas = describe "liftAllLambdas" $ do
  it "does nothing to an expression with no lambdas" $ do
    let a = TOther $ UType $ U 1 "a"
        e = LetT a [("x", VarT a "y", Nothing)] (AppT a (VarT a "f") (VarT a "x"))
  ...
  it "lifts an expression with one lambda" $ do
    ...
    newBindings `shouldBe` [("g", [VarPat b "x", VarPat a "w"], VarT b "x")]
  ...
  it "lifts an expression with two lambdas" $ do
    ...
    newBindings `shouldBe` [("f_1", [VarPat b "x", VarPat a "w"], VarT b "x"), ("f_2", [VarPat d "z", VarPat c "y"], VarT d "z")]
    result `shouldBe` expected
...
test_liftLet = describe "liftLet" $ do
  it "lifts a simple let" $ do
    ...
    let a = TOther $ UType (U 1 "a")
        ...
        result = liftLet emptyGenName mempty "x" (VarT a "y") (VarT a "x") Nothing hole
        expectedResult = LetT b [("x", VarT a "y", Nothing)] (AppT b (VarT (TOther (Fn a b)) "f") (VarT a "x"))
  ...
  it "lifts a nested let" $ do
    ...
    let a = TOther $ UType (U 1 "a")
        ...
        hole e = AppT b (AppT fxgType (AppT fxType (VarT fType "f") e) (VarT b "g")) (VarT a "h")
        result = liftLet emptyGenName mempty "x" (VarT a "y") (VarT a "x") Nothing hole
        expectedResult = LetT b [("x", VarT a "y", Nothing)] (AppT b (AppT fxgType (AppT fxType (VarT fType "f") (VarT a "x")) (VarT b "g")) (VarT a "h"))
    result `shouldBe` pure expectedResult
  it "lifts let that needs renaming" $ do
    -- f (let f = y in f) ==> let g = y in f g
    let a = TOther $ UType (U 1 "a")
        b = TOther $ UType (U 2 "b")
        hole = AppT b (VarT (TOther (Fn a b)) "f")
        genName = pure "g" :: Identity Name
        result = liftLet genName (Set.singleton "f") "f" (VarT a "y") (VarT a "f") Nothing hole
        expectedResult = LetT b [("g", VarT a "y", Nothing)] (AppT b (VarT (TOther (Fn a b)) "f") (VarT a "g"))
    result `shouldBe` pure expectedResult
